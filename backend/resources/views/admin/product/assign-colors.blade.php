        ...
        <div class="flex-grow-1 d-flex flex-column" style="background-color: #f1f4f8; overflow: hidden;">
            ...
            <main class="flex-grow-1 p-3 p-md-4" style="overflow-y: auto;">
                <div class="container-fluid">
                    ...
                    <!-- Page Header -->
                    <div class="mb-4">
                        <h4 class="fw-bold text-primary mb-1">Assign Colors</h4>
                        <p class="text-muted small mb-0">Choose which plain finishes customers can apply to: <strong>{{ $product->name }}</strong></p>
                    </div>
                    ...
                    <form action="{{ route('admin.products.colors.store', $product->product_id) }}" method="POST">
                        @csrf
                        ...
                        <div class="card border-0 shadow-sm rounded-4 mb-4">
                            <div class="card-header bg-white border-bottom-0 pt-3 pt-md-4 px-3 px-md-4 d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2">
                                <div>
                                    ...
                                    <p class="text-muted small mb-0 mt-1">Click a swatch to toggle its assignment to this product.</p>
                                </div>
                            ...
                            <div class="card-body p-3 p-md-4">
                                @if($colors->isEmpty())
                                    <div class="alert alert-warning">
                                        ...
                                        No colors available. Please <a href="{{ route('admin.colors.index') }}">add colors</a> first.
                                    </div>
                                @else
                                    <div class="row g-3">
                                        @php
                                            $assignedIds = $product->colors->pluck('color_id')->all();
                                        @endphp
                                        @foreach($colors as $color)
                                            @php $isChecked = in_array($color->color_id, $assignedIds); @endphp
                                            <div class="col-md-3 col-sm-4 col-6">
                                                <label class="color-card-label d-block h-100" style="cursor: pointer;">
                                                    <input type="checkbox"
                                                        name="colors[]"
                                                    ...
                                                    <div class="color-card card border-2 rounded-4 h-100 overflow-hidden {{ $isChecked ? 'border-primary shadow-sm' : 'border-transparent' }}">
                                                        ...
                                                        <div class="card-body p-2">
                                                            <div class="fw-bold text-dark small mb-0 text-truncate">{{ $color->name }}</div>
                                                            @if($color->price_modifier > 0)
                                                                <small class="text-warning">+₱{{ number_format($color->price_modifier, 2) }}</small>
                                                            @else
                                    ...
                                    <div class="alert alert-info mt-4 mb-0">
                                        ...
                                        <strong>Tip:</strong> Only colors assigned here appear in the customizer for this product.
                                        Assign none and every colour is offered. A customer picks either a colour or a texture,
                                        never both.
                                        @if(!$product->is_customizable)
                        ...
                        <div class="d-flex flex-wrap gap-2 justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary rounded-pill px-4">
                                <i class="bi bi-check-lg me-2"></i>Save Color Assignments
                            </button>
                        </div>
                    </form>
